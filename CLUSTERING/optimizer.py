...
import optimizers.abc as abc
import optimizers.woa as woa
import optimizers.de as de
import optimizers.abc_woa as abc_woa
import optimizers.abc_de as abc_de
import optimizers.awd as awd 
		...
		for j in range (0, len(objectivefunc)):		
			for h in range(len(dataset_List)):
				...
				for z in range (0,NumOfRuns):
					...
					if(color):
						print(colored("Dataset: "+str(dataset_List[h]),'green',attrs=['bold']))
						print(colored("Dimensions: "+str(f[h]+1) ,"green",attrs=['bold']))
						print(colored("Run no.: "+str(z),'white',attrs=['dark','bold']))
						print(colored("Iterations: "+str(Iterations),"white",attrs=['dark','bold']))
						print(colored("Population Size: "+str(PopulationSize),'magenta',attrs=['bold']))
					else:
						print("Dataset: " + dataset_List[h])
						print("Dimensions: " + str(f[h]+1)) 
						print("Run no.: " + str(z)) 
...
def selector(algo,objective_name, k, f, popSize,Iter, points, metric):
	...
	if(algo=="ABC_WOA_DE"):
		x=awd.ABCWDE(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric)
	if(algo=="ABC"):
		x=abc.ABC(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric)
	if(algo=="ABC_DE"):
		x=abc_de.ABC_DE(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric)
	if(algo=="WOA"):
		x=woa.WOA(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric)
	if(algo=="DE"):
		x=de.DE(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric)
	if(algo=="ABC_WOA"):
		x=abc_woa.ABCWOA(getattr(objectives, objective_name),lb,ub,k * f,popSize,Iter, k, points, metric)
	return x